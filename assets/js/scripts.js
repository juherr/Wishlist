            ...
            if (gift_title === '') {
                ...
                $.ajax({
                    ...
                    success: function (json) {
                        ...
                        var gift_url_code = '';
                        if (json.gift_url !== '') {
                            gift_url_code = '<a title="Lien vers le cadeau" href="' + json.gift_url + '" class="gift-link">' +
                        ...
                        if (json.gift_description !== '') {
                            gift_description_code = '<p class="gift-description">' + json.gift_description + '</p>'
                        }
                        ...
                        $this.parent().find('ul').append('<li>' +
                            ...
                            '        </svg>' +
                            '      </span>' +
                            '      <input type="text" name="gift-name" required placeholder="Désignation" value="' + gift_title + '">' +
                            '    </div>' +
                            '    <div class="wrapper-gift-input">' +
                            '      <span>' +
                            '        <svg viewBox="0 0 100 100" class="icon">' +
        ...
        $('body').on('submit', '.form-edit', function (e) {
            ...
            $.ajax({
                ...
                success: function (json) {
                    ...
                    if (gift_url !== '') {
                        $this.parent().find('.wrapper-title').append('<a title="Lien vers le cadeau" href="' + gift_url + '" class="gift-link">' +
                            '  <svg viewBox="0 0 100 100" class="icon">' +
                            '    <use xlink:href="#icon-link"></use>' +
                            '  </svg>' +
                            '</a>');
        ...
        $('body').on('submit', '#form-resa', function (e) {
            ...
            $.ajax({
                ...
                success: function (json) {
                    ...
                    $this.parent().append('<form action="' + route_booking_delete + '" id="cancel_resa" method="post">' +
                        '  <input type="hidden" value="' + json.gift_id + '" name="gift-id">' +
        ...
        $('body').on('submit', '#cancel_resa', function (e) {
            ...
            $.ajax({
                ...
                success: function (json) {
                    ...
                    $this.parent().append('<form action="' + route_booking_create + '" method="post" id="form-resa">' +
                        '  <input type="hidden" value="' + json.gift_id + '" name="gift-id">' +
